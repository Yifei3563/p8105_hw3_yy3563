...
The average tmax in January show a much larger spread compared to July. Both of the months show a consistent pattern. There are a few outliers below -10°C in January. These could be caused by extremely cold days. There is one outlier in July where the station was recorded below 15. 
...
citi_bike |> 
  ...
  geom_boxplot() +
  facet_grid(. ~ Year) +
  labs(title="Boxplot of duration by year, month and weekdays")
```
...
citi_bike |> 
  ...
  geom_boxplot() +
  facet_grid(. ~ Month) +
  labs(title="Boxplot of duration by member casual, bike type and month")
```
